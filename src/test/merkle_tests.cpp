...
BOOST_AUTO_TEST_CASE(merkle_stable_branch)
{
    ...
    for (uint32_t i = 0; i < leaves.size(); ++i) {
        std::vector<uint256> old_branch = ComputeMerklePath(leaves, i);
...
BOOST_AUTO_TEST_CASE(merkle_tree_constructor)
{
    ...
    invalid = true;
    BOOST_CHECK(zero.GetHash(&invalid) == hashZero);
    BOOST_CHECK_EQUAL(invalid, false);
    ...
    invalid = true;
    BOOST_CHECK(verify.GetHash(&invalid) == hashA);
    BOOST_CHECK_EQUAL(invalid, false);
    ...
    invalid = true;
    BOOST_CHECK(skip.GetHash(&invalid) == hashB);
    BOOST_CHECK_EQUAL(invalid, false);
...
BOOST_AUTO_TEST_CASE(merkle_tree_two_items)
{
...

    {
        ...
        invalid = true;
        BOOST_CHECK(res.GetHash(&invalid) == hashAB);
        BOOST_CHECK_EQUAL(invalid, false);
...
BOOST_AUTO_TEST_CASE(merkle_tree_three_items)
{
...

    {
        ...
        invalid = true;
        BOOST_CHECK(res.GetHash(&invalid) == hashAB_C);
        BOOST_CHECK_EQUAL(invalid, false);
        BOOST_CHECK(MerkleTree(zero, res) == res);
        BOOST_CHECK(MerkleTree(res, zero) == res);
    }
...
BOOST_AUTO_TEST_CASE(merkle_tree_three_items_reversed)
{
...

    {
        ...
        invalid = true;
        BOOST_CHECK(res.GetHash(&invalid) == hashD_EF);
        BOOST_CHECK_EQUAL(invalid, false);
        BOOST_CHECK(MerkleTree(zero, res) == res);
        BOOST_CHECK(MerkleTree(res, zero) == res);
    }
...
BOOST_AUTO_TEST_CASE(merkle_tree_four_items)
{
    ...
    uint256 hashAB_CD;
    hashAB_CD = MerkleHash_Sha256Midstate(hashAB, hashCD);
...

    {
        ...
        invalid = true;
        BOOST_CHECK(res.GetHash(&invalid) == hashAB_CD);
        BOOST_CHECK_EQUAL(invalid, false);
        BOOST_CHECK(MerkleTree(zero, res) == res);
        BOOST_CHECK(MerkleTree(res, zero) == res);
    }
...
BOOST_AUTO_TEST_CASE(merkle_tree_six_items_part1)
{
...

    {
        ...
        invalid = true;
        BOOST_CHECK(res.GetHash(&invalid) == hashAB_C__D_EF);
        BOOST_CHECK_EQUAL(invalid, false);
        BOOST_CHECK(MerkleTree(zero, res) == res);
        BOOST_CHECK(MerkleTree(res, zero) == res);
    }
...
BOOST_AUTO_TEST_CASE(fast_merkle_branch)
{
    ...
    const std::vector<uint256> leaves = {
      (HashWriter{} << static_cast<uint8_t>('a')).GetHash(),
      (HashWriter{} << static_cast<uint8_t>('b')).GetHash(),
      (HashWriter{} << static_cast<uint8_t>('c')).GetHash(),
    };
    const uint256 root = ComputeFastMerkleRoot(leaves);
    BOOST_CHECK(root == uint256{"35d7dea3df173ecb85f59ebb88b2003be3c94b576576b12eb8d017f9fc33b289"});
    {
    ...
    }
    {
        ...
        BOOST_CHECK(branch[0] == uint256{"a6e8f6cfa607807d35da463f0599aa0d8032dda4e5635c806098a9ed332b6279"});
