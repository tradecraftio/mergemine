...
static AuxWorkServer* GetServerFromChainId(const ChainId& chainid, const std::string& caller) EXCLUSIVE_LOCKS_REQUIRED(cs_mergemine)
{
    if (!g_mergemine.count(chainid)) {
        LogDebug(BCLog::MERGEMINE, "%s: error: unrecognized chainid with no active connection: 0x%s\n", caller, HexStr(chainid));
        return nullptr;
    }
...
static int SendAuxAuthorizeRequest(AuxWorkServer& server, const std::string& username, const std::string& password) EXCLUSIVE_LOCKS_REQUIRED(cs_mergemine)
{
    ...
    LogDebug(BCLog::MERGEMINE, "Sending stratum request to %s (%s) : %s", server.socket.ToStringAddrPort(), server.name, request);
    evbuffer *output = bufferevent_get_output(server.bev);
    if (evbuffer_add(output, request.data(), request.size())) {
        LogDebug(BCLog::MERGEMINE, "Sending stratum request failed. (Reason: %d, '%s')\n", errno, evutil_socket_error_to_string(errno));
    }
...
std::map<ChainId, AuxWork> GetMergeMineWork(const std::map<ChainId, std::pair<std::string, std::string> >& auth)
{
    ...
    for (const auto& item : auth) {
        ...
        AuxWorkServer* server = GetServerFromChainId(chainid, __func__);
        if (!server) {
            LogDebug(BCLog::MERGEMINE, "Requested work for chain 0x%s but no active connection to that chain.\n", HexStr(chainid));
            continue;
        }

        // Lookup the canonical address for the user.
        if (!server->aux_auth.count(username)) {
            LogDebug(BCLog::MERGEMINE, "Requested work for chain 0x%s but user \"%s\" is not registered.\n", HexStr(chainid), username);
            RegisterMergeMineClient(*server, username, password);
            continue;
        }
        ...
        if (!server->aux_work.count(address)) {
            LogDebug(BCLog::MERGEMINE, "No work available for user \"%s\" (\"%s\") on chain 0x%s\n", username, address, HexStr(chainid));
            continue;
        }
        LogDebug(BCLog::MERGEMINE, "Found work for user \"%s\" (\"%s\") on chain 0x%s\n", username, address, HexStr(chainid));
    ...
    for (auto& item : g_mergemine_conn) {
        ...
        if (!server.default_auth) {
            LogDebug(BCLog::MERGEMINE, "No default credentials for chain 0x%s\n", HexStr(server.aux_pow_path));
            continue;
        }
        ...
        if (ret.count(chainid)) {
            LogDebug(BCLog::MERGEMINE, "Already have work for chain 0x%s\n", HexStr(chainid));
            continue;
        }
        ...
        if (!server.aux_auth.count(username)) {
            ...
            LogDebug(BCLog::MERGEMINE, "Default credentials for chain 0x%s are not registered yet.\n", HexStr(chainid));
            RegisterMergeMineClient(server, username, password);
            continue;
        }
        ...
        const std::string& address = server.aux_auth[username];
        if (!server.aux_work.count(address)) {
            LogDebug(BCLog::MERGEMINE, "No work available for default user \"%s\" (\"%s\") on chain 0x%s\n", username, address, HexStr(chainid));
            continue;
        }
        // Add work for this chain.
        LogDebug(BCLog::MERGEMINE, "Adding work for default user \"%s\" (\"%s\") on chain 0x%s\n", username, address, HexStr(chainid));
        ret[chainid] = server.aux_work[address];
    }
...
static std::string GetRegisteredAddress(const AuxWorkServer& server, const std::optional<std::string>& username) EXCLUSIVE_LOCKS_REQUIRED(cs_mergemine)
{
    ...
    if (username.has_value()) {
        ...
        if (server.default_auth.has_value()) {
            ...
            LogDebug(BCLog::MERGEMINE, "Submitted work for chain 0x%s with default credentials, but auxiliary work server does not support default credentials; using \"%s\" as username.\n", HexStr(server.aux_pow_path), key);
        }
    }
    std::string ret;
    if (server.aux_auth.count(key)) {
        ...
        boost::trim(ret);
        LogDebug(BCLog::MERGEMINE, "Submitted work for chain 0x%s but user \"%s\" is not registered; assuming address is \"%s\".\n", HexStr(server.aux_pow_path), key, ret);
    }
    return ret;
}
...
void SubmitAuxChainShare(const ChainId& chainid, const std::optional<std::string>& username, const AuxWork& work, const AuxProof& proof)
{
    ...
    if (!server) {
        ...
        if (g_second_stage.count(chainid)) {
            LogDebug(BCLog::MERGEMINE, "Auxiliary work server for chain 0x%s is not connected; discarding share.\n", HexStr(chainid));
...
static UniValue stratum_mining_aux_notify(AuxWorkServer& server, const UniValue& params) EXCLUSIVE_LOCKS_REQUIRED(cs_mergemine)
{
    ...
    if (g_second_stage.count(server.aux_pow_path)) {
        LogDebug(BCLog::MERGEMINE, "Removing second-stage work unit for chain 0x%s", HexStr(server.aux_pow_path));
    ...
    for (const std::string& address : commits.getKeys()) {
        ...
        LogDebug(BCLog::MERGEMINE, "Got commitment for aux address \"%s\" on chain 0x%s: 0x%s %s,%d\n", address, HexStr(server.aux_pow_path), commit.ToString(), HexInt4(bits), bias);
...
static UniValue stratum_mining_notify(AuxWorkServer& server, const UniValue& params) EXCLUSIVE_LOCKS_REQUIRED(cs_mergemine)
{
    ...
    if (g_second_stage.count(server.aux_pow_path)) {
        LogDebug(BCLog::MERGEMINE, "Replacing second stage work unit for chain 0x%s\n", HexStr(server.aux_pow_path));
    }
...
static void merge_mining_read_cb(bufferevent *bev, void *ctx)
{
    ...
    if (!g_mergemine_conn.count(bev)) {
        LogDebug(BCLog::MERGEMINE, "Received read notification for unknown auxiliary work source on connection 0x%x\n", (size_t)bev);
        return;
    }
    ...
    while (!done && (cstr = evbuffer_readln(input, &len, EVBUFFER_EOL_CRLF))) {
        ...
        LogDebug(BCLog::MERGEMINE, "Received line of data from auxiliary work source stratum+tcp://%s (%s) : %s\n", server.socket.ToStringAddrPort(), server.name, line);
        ...
        try {
            if (isreply && val["id"].getInt<int>() == -1) {
                ...
                LogDebug(BCLog::MERGEMINE, "Setting extranonce1 to \"%s\" and extranonce2_size to %d from stratum+tcp://%s (%s)\n", HexStr(extranonce1), extranonce2_size, server.socket.ToStringAddrPort(), server.name);
        ...
        if (isreply) {
            try {
                ...
                if (server.aux_auth_jreqid.count(id)) {
                    ...
                    LogDebug(BCLog::MERGEMINE, "Mapping username \"%s\" to remote address \"%s\" for stratum+tcp://%s (%s)\n", username, address, server.socket.ToStringAddrPort(), server.name);
                    server.aux_auth[username] = address;
                    server.aux_auth_jreqid.erase(id);
                }
            } catch (const std::exception& e) {
                LogDebug(BCLog::MERGEMINE, "Error while handling response from stratum+tcp://%s (%s): %s\n", server.socket.ToStringAddrPort(), server.name, e.what());
            }
            continue;
        }
        ...
        try {
            ...
            // Compose reply
            reply = JSONRPCReplyObj(result, NullUniValue, jreq.id, jreq.m_json_version).write() + "\n";
        } catch (const UniValue& objError) {
            // If a JSON object is thrown, we can encode it directly.
            reply = JSONRPCReplyObj(NullUniValue, objError, jreq.id, jreq.m_json_version).write() + "\n";
        } catch (const std::exception& e) {
            // Otherwise turn the error into a string and return that.
            reply = JSONRPCReplyObj(NullUniValue, JSONRPCError(RPC_PARSE_ERROR, e.what()), jreq.id, jreq.m_json_version).write() + "\n";
        }

        // Send reply
        LogDebug(BCLog::STRATUM, "Sending stratum response to stratum+tcp://%s (%s) : %s", server.socket.ToStringAddrPort(), server.name, reply);
    ...
    if (done) {
        LogDebug(BCLog::MERGEMINE, "Closing initial stratum connection to stratum+tcp://%s (%s)\n", server.socket.ToStringAddrPort(), server.name);
...
static void merge_mining_event_cb(bufferevent *bev, short what, void *ctx)
{
    ...
    // Lookup the client record for this connection
    if (!g_mergemine_conn.count(bev)) {
        LogDebug(BCLog::MERGEMINE, "Received event notification for unknown auxiliary work source on connection 0x%x\n", (size_t)bev);
        return;
    }
    AuxWorkServer& server = g_mergemine_conn[bev];
    // Report the reason why we are closing the connection.
    if (what & BEV_EVENT_ERROR) {
        LogDebug(BCLog::MERGEMINE, "Error detected on initial stratum connection to stratum+tcp://%s (%s)\n", server.socket.ToStringAddrPort(), server.name);
    }
    if (what & BEV_EVENT_EOF) {
        LogDebug(BCLog::MERGEMINE, "Remote disconnect received on stratum connection to stratum+tcp://%s (%s)\n", server.socket.ToStringAddrPort(), server.name);
    }
    ...
    if (what & (BEV_EVENT_EOF | BEV_EVENT_ERROR)) {
        ...
        if (g_second_stage.count(server.aux_pow_path)) {
            LogDebug(BCLog::MERGEMINE, "Removing second-stage work unit for chain 0x%s from stratum+tcp://%s (%s) (reason: disconnect)\n", HexStr(server.aux_pow_path), server.socket.ToStringAddrPort(), server.name);
            g_second_stage.erase(server.aux_pow_path);
        }
...
void MergeMiningManagerThread()
{
    ...
    if (g_mergemine_conn.empty() && g_mergemine_noconn.empty()) {
        LogDebug(BCLog::MERGEMINE, "No merge-mining connections to manage, exiting event dispatch thread.\n");
        return;
    }
    ...
    LogDebug(BCLog::MERGEMINE, "Entering merge-mining event dispatch loop\n");
    while (!g_shutdown) {
    ...
    LogDebug(BCLog::MERGEMINE, "Exited merge-mining event dispatch loop\n");
...
void StopMergeMining()
{
    ...
    for (auto& server : g_mergemine_conn) {
        LogDebug(BCLog::MERGEMINE, "Closing stratum connection to stratum+tcp://%s (%s) due to process termination\n", server.second.socket.ToStringAddrPort(), server.second.name);
