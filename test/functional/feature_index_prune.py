    ...
    def run_test(self):
        ...
        for node in self.nodes[:2]:
            with node.assert_debug_log(['limited pruning to height 689']):
                ...
                assert_equal(pruneheight_new, 220)
        ...
        self.generate(self.nodes[0], 830 - 700)
        self.sync_index(height=830)
        ...
        self.generate(self.nodes[0], 1500 - 830)

        self.log.info("prune exactly up to the indices best blocks while the indices are disabled")
        for i in range(3):
            pruneheight_2 = self.nodes[i].pruneblockchain(1000)
            assert_equal(pruneheight_2, 830 - 1)
        ...
        for node in self.nodes[:2]:
            with node.assert_debug_log(['limited pruning to height 2489']):
                ...
                assert_equal(pruneheight_new, 2047)
