...
#include <node/abort.h> // for node::AbortNode
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        if (txin.prevout.hash == final_tx.hash && txin.prevout.n < final_tx.size) {
            return state.Invalid(TxValidationResult::TX_SPEND_BLOCK_FINAL, "spend-block-final-txn");
        }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (enforce_block_final && !any_prev_final_utxos) {
        ...
        std::atomic<int> exit_status = EXIT_FAILURE;
        node::AbortNode(exit_status, strprintf("%s: prior block-final tx hash %s not found; corruption likely!", __func__, entry.hash.GetHex()), _("Database corruption likely.  Try restarting with `-reindex=1`."));
    ...
    if (enforce_block_final) {
        ...
        if (coin.IsSpent()) {
            // Should never happen.
            std::atomic<int> exit_status = EXIT_FAILURE;
            node::AbortNode(exit_status, strprintf("%s: unspent output of prior block-final tx outpoint %s:0 not found; corruption likely!", __func__, entry.hash.GetHex()), _("Database corruption likely.  Try restarting with `-reindex=1`."));
            return false; // unreachable
        }
